...
class Game_Board:
    def __init__(self, rows, cols, mines):
        ...
        self.is_revealed_map = [[0 for x in range(cols)] for y in range(rows)]
    ...
    def get_new_revealed_game_board(self, row, col):
        stack = [[row, col]]
        while len(stack) > 0:
            cur_tile = stack.pop()
            row=cur_tile[0]
            col=cur_tile[1]
            if self.is_revealed_map[row][col] == 0:
                if self.map[row][col] == 0:
                    ...
                    if col-1 >= 0:
                        if row+1 < self.rows:
                            stack.append([row+1, col-1])
                        stack.append([row, col-1])
                    self.is_revealed_map[row][col] = 1
            self.is_revealed_map[row][col] = 1
